...
echo ========================================
echo CityHoppa - prepare self-update files
echo ========================================
echo.
echo Builds the release APK and writes build\update.json.
echo Upload both files to https://trimline.co.ke/apps/CityHoppa/
echo (or use build_and_deploy.ps1 to upload automatically).
echo.
...
for /f "tokens=2 delims=+" %%a in ("%VERSION_FULL%") do set VERSION_CODE=%%a
