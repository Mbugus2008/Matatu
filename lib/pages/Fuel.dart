...
class _FuelScreenState extends State<FuelScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: GetBuilder<FuelController>(
        init: FuelController(),
        builder: (ctrl) => FloatingActionButton.extended(
          onPressed: ctrl.isUpdating ? null : () => ctrl.updateDepot(),
          backgroundColor: _primaryColor(),
...
class _FuelState extends State<Fuel> {
      ...
      child: Column(
        ...
        children: [
          _buildTextField(
            ...
            'Litres',
            (value) {
              vehicle.Total_litres = double.tryParse(value) ?? 0;
              vehicle.dirty = true;
            },
  ...
  Widget _buildFinancialInfo(DepotFuel vehicle) {
    final totalCollection = vehicle.Total_Collection ?? 0;
    final mgmtTarget = vehicle.Management_Target ?? 0;
    final fuelCost = vehicle.Fuel ?? 0;
    final amountPaid = vehicle.Amount_Paid ?? 0;
    // If no collection, everything is zero
    final subNet = totalCollection == 0 ? 0.0 : totalCollection - mgmtTarget;
    final unpaid = totalCollection == 0
        ? 0.0
        : (fuelCost - amountPaid).clamp(0.0, double.infinity);
    final bal = totalCollection == 0
        ? 0.0
        : ((subNet - unpaid) > 0 ? 0.0 : (subNet - unpaid));
    final offload = totalCollection == 0
        ? 0.0
        : (subNet - unpaid > 0 ? subNet - unpaid : 0.0);
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
                NumberFormat.currency(locale: 'en_US', symbol: 'Kshs ')
