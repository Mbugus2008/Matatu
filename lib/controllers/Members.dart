...
class MemberController extends GetxController {
  ...
  /// Detaches a single crew member from [vehicle] in the database and the local
  /// cache.
  ///
  /// Prefer this over [clearcrew] when replacing a driver/conductor: several
  /// crew rows can share one vehicle, and clearing the whole vehicle discards
  /// the rest of them.
  Future<void> detachcrew(String vehicle, String? crew) async {
    ...
    for (final m in Get.find<MemberController>().allMembers) {
      if (m.No == crew && m.Vehicle == vehicle) {
        m.Vehicle = '';
      }
    }
    update();
